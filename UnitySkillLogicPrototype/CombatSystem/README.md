...
체력이 0 이하가 되면 Collider를 비활성화하고 `Died` 이벤트와 사망 애니메이션을 실행합니다.
...
`screenPosition.z <= 0`이면 Enemy가 카메라 뒤에 있다고 판단하여 Health Bar를 숨깁니다.
...
SkillButton이 별도의 쿨타임을 계산하지 않고 Runtime 상태를 그대로 사용하여 실제 사용 가능 상태와 UI 표시의 기준을 통일했습니다.

### `SkillRangeIndicator` — 적용 범위 시각화

스킬 실행 시 원형 Indicator를 활성화하고, `radius * 2`를 지름으로 사용해 크기를 조정합니다.

지정된 시간이 지나면 Coroutine으로 Indicator를 비활성화합니다.
...
`SkillButton`은 매 Frame Runtime 상태를 조회합니다. 스킬 수가 증가하는 경우 Runtime 상태 변경 이벤트를 추가해 UI를 필요한 시점에만 갱신하는 방식과 비교할 수 있습니다.

`SkillRangeIndicator`는 짧은 시간 안에 여러 번 호출될 경우 이전 Hide Coroutine이 새 표시를 먼저 끌 수 있으므로, 재호출 시 기존 Coroutine을 중단하는 정책을 추가할 수 있습니다.
