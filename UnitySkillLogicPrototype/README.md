...
게임 디자이너가 작성한 스킬 명세를 `Data → Runtime State → Validation → Execution → Feedback` 단계로 분해하고,  
Normal, DoT, Area 스킬의 실행 흐름과 예외 조건을 작은 Unity 환경에서 구현·검증했습니다.

본 저장소에는 전체 Unity 프로젝트가 아니라, 포트폴리오 검토에 필요한 **직접 작성한 핵심 C# 코드**를 정리했습니다.
...
| **프로젝트 유형** | Combat Logic R&D Prototype |
...
> **스킬의 설정 데이터와 실행 중 상태를 분리하고, 서로 다른 스킬 규칙을 공통 실행 흐름 안에서 처리할 수 있는가?**
...
유효한 단일 대상에게 설정된 간격에 따라 지속 피해를 적용합니다.
...
정상 실행뿐 아니라 실패 및 예외 조건에서 상태가 일관되게 유지되는지 확인했습니다.

| 검증 시나리오 | 기대 동작 |
| --- | --- |
| 스킬 사용 가능 상태 | 즉시 실행 후 쿨타임 시작 |
| 쿨타임 중 재사용 | 실행 차단 |
| 대상 없음 | 스킬 실패 / 쿨타임 미적용 |
| 대상이 사거리 밖 | 스킬 실패 / 쿨타임 미적용 |
| 사망한 대상 | Target 후보에서 제외 |
| Normal Skill | 단일 대상 즉시 피해 |
| DoT Skill | 설정된 간격에 따라 지속 피해 |
...
후속 단계에서는 Targeting Rule과 Skill Effect를 독립 실행 단위로 분리하고, 여러 Effect를 조합할 수 있는 구조를 검토할 예정입니다.
